...
int email_sort_path(const void *a, const void *b)
{
  const struct Email **e_a = (const struct Email **) a;
  const struct Email **e_b = (const struct Email **) b;

  return strcmp((*e_a)->path, (*e_b)->path);
}

int email_sort_inode(const void *a, const void *b)
{
  const struct Email **e_a = (const struct Email **) a;
  const struct Email **e_b = (const struct Email **) b;

  const struct Maildir *edata_a = (*e_a)->edata;
  const struct Maildir *edata_b = (*e_b)->edata;

  return (edata_a->inode - edata_b->inode);
}

void maildir_delayed_parsing2(struct Mailbox *m, struct Vector *v, struct Progress *progress)
{
  char fn[PATH_MAX];
  bool sort = false;
  ...
  {
    struct Email *e = v->data[i];
    struct Maildir *edata = e->edata;

    if (edata->header_parsed)
    {
      printf("SKIP: %s\n", edata->canon_fname);
      continue;
    }
    ...
    if (!sort)
    {
      // qsort(v->data, v->size, sizeof(struct Email *), email_sort_inode);
      //QWQ p = skip_duplicates(p, &last);
    }

    snprintf(fn, sizeof(fn), "%s/%s", mailbox_path(m), e->path);
    ...
    {
      hce.email->old = e->old;
      hce.email->path = mutt_str_strdup(e->path);
      email_free(&e);
      e = hce.email;
      if (m->type == MUTT_MAILDIR)
        maildir_parse_flags(e, fn);
    }
    else
#endif
    {
      printf("parse: %10lu %s\n", edata->inode, fn);
      if (maildir_parse_message(m->type, fn, e->old, e))
      {
        edata->header_parsed = true;
#ifdef USE_HCACHE
        if (m->type == MUTT_MH)
        {
          key = e->path;
  ...
  //QWQ mh_sort_natural(m, md);
  qsort(v->data, v->size, sizeof(struct Email *), email_sort_path);
}
...
int maildir_parse_dir2(struct Mailbox *m, struct Vector *v, const char *subdir, struct Progress *progress)
{
  ...
  while (((de = readdir(dirp))) && (SigInt != 1))
  {
    ...
    if (m->verbose && progress)
      mutt_progress_update(progress, v->size, -1);
...
int mh_read_dir(struct Mailbox *m, const char *subdir)
{
  ...
  struct Vector *v = vector_new(sizeof(struct Email *));

  if (maildir_parse_dir2(m, v, subdir, &progress) < 0)
    return -1;
  ...
  if (m->verbose)
  {
    ...
    mutt_progress_init(&progress, msg, MUTT_PROGRESS_READ, v->size);
  }
  struct Maildir *md = NULL;
  maildir_delayed_parsing2(m, v, &progress);
#if 0
  if (m->type == MUTT_MH)
  {
  ...
  vector_free(&v, (vector_item_free_t) email_free);
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < m->email_max; i++)
  {
    ...
    struct Maildir *edata = maildir_edata_get(e);
    if (!e || !edata)
      continue;
    printf("%10ld %s\n", edata->inode, e->path);
  }
