...
void test_nm_path2_canon(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_equal(path.canon, second));
    }
    FREE(&path.canon);
  }
}
...
void test_nm_path2_parent(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    TEST_CHECK(rc == tests[i].retval);
    TEST_CHECK(mutt_str_equal(parent ? parent->orig : NULL, second));
  }
}
...
void test_nm_path2_pretty(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc > 0)
    {
      ...
      TEST_CHECK(mutt_str_equal(path.pretty, second));
    }
  }
}
...
void test_nm_path2_tidy(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_equal(path.orig, second));
    }
    FREE(&path.orig);
  }
}
