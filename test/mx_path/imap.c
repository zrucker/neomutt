...
void test_imap_path2_canon(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_equal(path.canon, tests[i].second));
    }
    FREE(&path.canon);
  }
}
...
void test_imap_path2_parent(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_equal(parent->orig, tests[i].second));
...
void test_imap_path2_pretty(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc > 0)
    {
      ...
      TEST_CHECK(mutt_str_equal(path.pretty, tests[i].second));
    }
  }
}
...
void test_imap_path2_tidy(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_equal(path.orig, tests[i].second));
    }
    FREE(&path.orig);
  }
}
