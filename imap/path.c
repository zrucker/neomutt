...
int imap_path2_compare(struct Path *path1, struct Path *path2)
{
  ...
  bool i1 = mutt_istr_equal(url1->path, "INBOX");
  bool i2 = mutt_istr_equal(url2->path, "INBOX");
  if (i1 && !i2)
  {
    rc = -1;
    goto done;
...
int imap_path2_parent(const struct Path *path, char delim, struct Path **parent)
{
  ...
  if ((url->path[0] == '\0') || mutt_str_equal(url->path, "INBOX"))
    goto done;
  ...
  *parent = mutt_path_new();
  (*parent)->orig = mutt_str_dup(mutt_b2s(buf));
...
int imap_path2_pretty(struct Path *path, const char *folder)
{
  ...
  if (url1->scheme != url2->scheme)
    goto done;
  if (!mutt_istr_equal(url1->host, url2->host))
    goto done;
...
int imap_path2_tidy(struct Path *path)
{
  ...
  if (!url->path || (url->path[0] == '\0') ||
      ...
      mutt_istr_equal(url->path, "inbox"))
  {
    url->path = "INBOX";
  }
